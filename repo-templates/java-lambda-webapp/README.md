# Java Lambda web app template

A team's starting point for a service that runs on AWS Lambda rather than on a
cluster: a Java 21 API in a container behind API Gateway, a DynamoDB table, and a
...
**Why this one exists beside [`go-lambda-webapp`](../go-lambda-webapp):** it is
the same stack — the same four plans, the same seven components, the same
pipeline — with the API written in Java rather than Go, for a team whose
services are Java. The web app and the smoke check are unchanged, because the
API answers the same paths with the same JSON.

The function is plain Java 21 on AWS's `public.ecr.aws/lambda/java:21` base
image: `aws-lambda-java-events` for the API Gateway request, the AWS SDK v2
DynamoDB client on the JDK's own HTTP client, and Jackson. There is no Spring,
because everything on the classpath is read on a cold start. The plan defaults
the function to 1024 MB for the same reason: Lambda gives CPU in proportion to
memory, and starting a JVM is CPU-bound.
...
| `RepoTemplate-java-lambda-webapp.yaml` | Registers the template with Wayfinder. |
