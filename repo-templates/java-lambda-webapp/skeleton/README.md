...
A Java 21 API running as a container on AWS Lambda, a DynamoDB table behind it, and a
...
| | |
| --- | --- |
| `src/main/java/com/example/orders/Handler.java` | The Lambda entry point, and the routes it answers |
| `src/main/java/com/example/orders/Order.java` | The order record and the rules it must satisfy |
| `src/main/java/com/example/orders/DynamoOrderStore.java` | The DynamoDB store behind the API |
| `src/test/` | JUnit tests, run by `make test` with no AWS account |
| `pom.xml` | The build. `make build` leaves the classes and runtime jars the image copies in `target/` |
...
- **apigateway** — one `ANY /api/{proxy+}` route to the function, which routes
  requests itself. Adding an endpoint is a change to `Handler.java` and nothing else.
...
You need a JDK 21 and Maven for this part.

```bash
make test     # JUnit, with an in-memory store standing in for DynamoDB
make lint     # compiles with -Xlint:all -Werror
make build    # target/classes and target/dependency, which the image copies
```

## Deploying it yourself

You need `make`, the `aws` CLI, `curl` and `jq`, plus `docker` if the image for
your `RELEASE` is not in ECR yet and `node` to build the web app — the image
build compiles the Java inside Docker, so no JDK is needed for a deploy: `pushimage`,
...
`RELEASE` tags the image and is written into the jar's `release.properties`, so
`/api/healthz` on the running function names the commit it came from. It also decides whether
...
`pushimage` and `deployweb` need docker, make and node when there is
