...
# deliberate: this repository's stack builds its two container images in a
# LocalAction, which runs on the machine driving the deploy. That machine
# therefore needs docker, make and the AWS CLI, which a GitHub-hosted ubuntu
# runner already has and the toolbox image does not.
set -euo pipefail
