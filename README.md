...
| [`repo-templates/aws-msk-kafka/`](./repo-templates/aws-msk-kafka/README.md) | A repository template for shared infrastructure: one MSK Serverless Kafka cluster per environment, which application stacks find by its `Environment` and `KafkaCluster` tags |